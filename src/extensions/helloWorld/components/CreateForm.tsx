...
//UI components
import { DatePicker, Text, DayOfWeek } from 'office-ui-fabric-react';
...
/*const calendarProps: ICalendarProps = {
  restrictedDates:[DayOfWeek.Friday]
}*/
...
export default function FormSolicitacaoFerias({
  ...
  const _onChangeSalario = (ev?: React.FormEvent<HTMLElement | HTMLInputElement>, isChecked?: boolean)
    ...
    const _onChangeDataInicio = (value?: Date): void => {
      console.log(value.getDay() === DayOfWeek.Friday);
      //if ((value.getDay() === DayOfWeek.Friday) === true) {
        if ((value.getDay() === DayOfWeek.Friday) == true || (value.getDay() === DayOfWeek.Saturday) === true || (value.getDay() === DayOfWeek.Sunday) === true){
  ...
  /*const isFriday = (date: Date | null): boolean => { return date && date.getDay() === DayOfWeek.Friday; };*/
  ...
  return (
    <Stack
      ...
      <Stack
        ...
        //disabled={formData.DataInicio && formData.DataInicio.getDay() === DayOfWeek.Friday}
        />
        <DatePicker
          label="Data de Fim"
          styles={datePickerStyles}
